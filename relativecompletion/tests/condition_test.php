...
/**
 * Unit tests for the relativecompletion condition.
 *
...
use availability_relativecompletion\condition;

global $CFG;
require_once($CFG->libdir . '/completionlib.php');

/**
 * Unit tests for the relative completion condition.
 *
 * @package availability_relativecompletion
 * @copyright 2014 The Open University
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @group availability_relativecompletion
 */
class availability_relativecompletion_condition_testcase extends advanced_testcase {
    /**
    ...
    public function test_in_tree() {
        ...
        $page = $generator->get_plugin_generator('mod_page')->create_instance(
                array('course' => $course->id, 'relativecompletion' => COMPLETION_TRACKING_MANUAL));
        ...
        // Set the idtype to 1 (= activity).
        $idtype = 1;

        $structure = (object)array('op' => '|', 'show' => true, 'c' => array(
                (object)array('type' => 'relativecompletion', 'idtype' => $idtype,
    ...
    public function test_constructor() {
        ...
        $cond = new condition($structure);
        $this->assertEquals('{completion:id42 COMPLETE}', (string)$cond);

        $structure->e = COMPLETION_COMPLETE_PASS;
        $cond = new condition($structure);
        $this->assertEquals('{completion:id42 COMPLETE_PASS}', (string)$cond);

        $structure->e = COMPLETION_COMPLETE_FAIL;
        $cond = new condition($structure);
        $this->assertEquals('{completion:id42 COMPLETE_FAIL}', (string)$cond);

        $structure->e = COMPLETION_INCOMPLETE;
        $cond = new condition($structure);
        $this->assertEquals('{completion:id42 INCOMPLETE}', (string)$cond);
    }
    ...
    public function test_save() {
        $structure = (object)array('id' => 42, 'e' => COMPLETION_COMPLETE);
        $cond = new condition($structure);
    ...
    public function test_usage() {
        ...
        $cond = new condition((object)array(
                'id' => (int)$pagecm->id, 'e' => COMPLETION_COMPLETE));
        ...
        $cond = new condition((object)array(
                'id' => (int)$pagecm->id, 'e' => COMPLETION_INCOMPLETE));
        ...
        // the other three.
        $cond = new condition((object)array(
                'id' => (int)$assigncm->id, 'e' => COMPLETION_INCOMPLETE));
        $this->assertTrue($cond->is_available(false, $info, true, $user->id));
        $this->assertFalse($cond->is_available(true, $info, true, $user->id));

        $cond = new condition((object)array(
                'id' => (int)$assigncm->id, 'e' => COMPLETION_COMPLETE));
        $this->assertFalse($cond->is_available(false, $info, true, $user->id));
        $this->assertTrue($cond->is_available(true, $info, true, $user->id));

        // Check $information for COMPLETE_PASS and _FAIL as we haven't yet.
        $cond = new condition((object)array(
                'id' => (int)$assigncm->id, 'e' => COMPLETION_COMPLETE_PASS));
        $this->assertFalse($cond->is_available(false, $info, true, $user->id));
        $information = $cond->get_description(false, false, $info);
        $information = \core_availability\info::format_info($information, $course);
        ...
        $cond = new condition((object)array(
                'id' => (int)$assigncm->id, 'e' => COMPLETION_COMPLETE_FAIL));
        $this->assertFalse($cond->is_available(false, $info, true, $user->id));
        $information = $cond->get_description(false, false, $info);
        $information = \core_availability\info::format_info($information, $course);
        ...
        $cond = new condition((object)array(
                'id' => ($assigncm->id + 100), 'e' => COMPLETION_COMPLETE));
        $this->assertFalse($cond->is_available(false, $info, true, $user->id));
        $information = $cond->get_description(false, false, $info);
        $information = \core_availability\info::format_info($information, $course);
        ...
        $this->assertFalse($cond->is_available(true, $info, true, $user->id));
        $cond = new condition((object)array(
                'id' => ($assigncm->id + 100), 'e' => COMPLETION_INCOMPLETE));
        $this->assertFalse($cond->is_available(false, $info, true, $user->id));
    }
    ...
    public function test_completion_value_used() {
        ...
        availability_relativecompletion\condition::wipe_static_cache();
        ...
        $DB->set_field('course_modules', 'availability',
                '{"op":"|","show":true,"c":[' .
                '{"type":"completion","e":1,"id":' . $page1->cmid . '}]}',
                array('id' => $page3->cmid));
        $DB->set_field('course_sections', 'availability',
                '{"op":"|","show":true,"c":[' .
                '{"type":"completion","e":1,"id":' . $page2->cmid . '}]}',
                array('course' => $course->id, 'section' => 1));

        // Now check: nothing depends on page3 but something does on the others.
        $this->assertTrue(availability_relativecompletion\condition::completion_value_used(
                $course, $page1->cmid));
        $this->assertTrue(availability_relativecompletion\condition::completion_value_used(
                $course, $page2->cmid));
        $this->assertFalse(availability_relativecompletion\condition::completion_value_used(
                $course, $page3->cmid));
    }
